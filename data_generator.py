...
def run_bowtie(bowtie_index, contents, frequency_tree):
    ...
    for i in range(len(contents)):
        ...
        for j in range(reads_to_be_analized//reads_per_random_index):
            commands.append(["time", "bowtie2", "-x ", bowtie_index, " --skip", str(random.randint(0, number_of_spots)),"--mm", "--upto", str(reads_per_random_index), "--no-hd", "--sra-acc", contents[i][0], ">>", "temp" + str(j) +".sam"])
