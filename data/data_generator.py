...
def run_bowtie(bowtie_index, contents, frequency_tree):
    ...
    for i in range(len(contents)):
        ...
        args = shlex.split("sudo /software/bowtie2/bowtie2 -x " +  bowtie_index + " --sra-acc " + contents[i][1] + " -sample-sra " + str(reads_to_be_analized) + " --threads 4 >> /root/temp.sam")
        print("ARGS = " + str(args))
        subprocess.call(args,  shell=True)
        # args = shlex.split("/software/bowtie2/bowtie2 -x "+  bowtie_index  + " --sra-acc " + contents[i][1])
        # subprocess.call(args,  shell=True)
